...
- 433c9ab: a11y audit: a fixture the site does not define is a `warn`, not a missing route (#900)
...
- d01e333: Pin four transitive advisories, two of them on the published runtime surface
...
- 1a29e13: `db dump` no longer emits a backup whose origin manifest disagrees with its own INSERTs. The
...
- 9cd0e49: New standalone recipe: `reddoor-maint match-harness [site] --ref <url>` installs the matching harness — the `/dev/match/[uid]` route, the `matching/` gate scripts and their one read layer, the site records, and the `.gitignore` / `.prettierignore` / `CLAUDE.md` blocks — in a single commit on its own branch.
...
- 8325d82: `prismic-ci` now resolves a positional checkout, and `launch` refuses to draft while a site's matching twin is live.
...
- 609aed8: Prospect accuracy: a near miss no longer matches a business out of its own
...
- cdff340: Prospect audit: the sitemap check counted a site's downloads as pages it was
...
- bb531e8: The prospect audit's check battery: 46 site checks where the report carried 12, each with four states (`pass` / `fail` / `unmeasured` / `not-applicable`) so that our own gaps and a site's genuine not-applicables leave the denominator instead of being printed as their defects.
