...
3. **Grid bands get a structural signature diff; smart slices get a payload-completeness check.** Hero/TitleBand/RichText intentionally carry no manifest tree (text is in the page doc, gated by `grid-slice.test.ts`), so re-checking their structure would false-positive. Gallery/MediaFull/Video/background/map are checked for presence/count. **SplitFeature is the exception among smart slices:** its `text` side is a full node subtree (band 1 of the-pointe nests media inside it), serialized via `renderNode` which drops unresolved media — so the gate signature-checks `bp.split.text` against `spec.text` (`sigOf`) exactly like a Grid tree, in addition to the presence + side/ratio checks. (Earlier drafts claimed presence/count was "the only fidelity smart slices can lose"; that was wrong for SplitFeature's nested media, caught in final review.)
