...
### Post-review closures (same day)

- Final review found the spec's threads contract ("fix-list grouped by page/source") was not delivered — the renderer now groups threads under `## <page>` headings keyed by `originalUrl ?? canonicalUrl` (thread objects carry `originalUrl`/`canonicalUrl`/`proxyUrl`; there is NO plain `url` field; "(no page)" fallback). Live-verified on a second throwaway pin, since deleted.
- Dispatcher/parser shipped hardened beyond the Task 1 scaffold: strict unknown-flag and surplus-positional rejection naming the offender, `--help` to stdout exit 0 vs no-args exit 1, missing value for `--query`/`--reply` dies before any command runs, top-level catch → exit 4, env key read once per process.
- Unresolved counts in `list` come from `activeThreads` on `GET /markups` directly, not the spec's `GET /markups/:id/view-modes` route (fewer calls, same number, live-verified).
- OPEN verification item: the spec's "confirm the reply + resolve state in the MarkUp UI" step was satisfied via API read-back, not a human look at the web UI. Whether a `{content}`-posted reply renders correctly to a designer in the browser (the UI stores rich text as Quill deltas in `messageJson`) is unverified until someone glances at a CLI-posted reply during the first real review round. Everything else about the round-trip is API-verified.
