...
Expected: non-zero. **Measured 2026-09-08: it is a 500, not a 404** — `Error: [function at(..)] unexpected field 'my.page.uid'`. The Content API rejects the PREDICATE rather than returning no document, so none of the 404 chain is reached: `page-load.ts:30-31` maps only `NotFoundError`, so this rethrows unmapped, and `svelte.config.js:34` rethrows again because its escape clause is `status === 404`. A `RepositoryNotFoundError` instead means the repository NAME is wrong, not that the document is missing — it extends `NotFoundError` and `page-load.ts:30` deliberately lets it through.

**Corrected later the same day, and this is the version to act on.** The first correction said this error means the repository name is right and the `page` custom type has never been pushed. That is wrong, and acting on it costs a wasted round trip re-pushing a type that is already there. Measured against `29-navy` with the type registered: `/api/v2` lists `"types":{"form_replies":…,"page":"Page"}`, matching the repo's own `customtypes/page/index.json` with all nine slices — and `at(my.page.uid,"home")` against the master ref STILL returns HTTP 400 `unexpected field 'my.page.uid'`, byte-identical to the same query for `my.nosuchtype.uid`, a type that has never existed. The queryable field set is derived from PUBLISHED content, so the error means **no published document of that type**, a condition that a missing type and a registered-but-unpopulated type satisfy equally. The build error cannot tell them apart, so do not diagnose from it. What does tell them apart:

```bash
curl -s https://<repo>.prismic.io/api/v2 | python3 -c "import sys,json;print(json.load(sys.stdin)['types'])"
```

A listed type plus that build error means the document is missing, not the model. **The signal runs one way only.** A type ABSENT from that map does not prove it was never pushed. Note the host above is NOT the `cdn.` one: measured 2026-09-08, `…cdn.prismic.io/api/v2` sets `cache-control: max-age=0, no-store` at the origin and CloudFront still answered an immediate second call with `x-cache: Hit from cloudfront`, so that map can be a snapshot from before a push — the 29-navy session read `"types":{}` from it while the Custom Types API listed `page`, and drew the wrong conclusion from it. The bare host returns no CloudFront headers. Confirm any ABSENCE against `customtypes.prismic.io` before concluding a model is missing; a presence is safe either way, since the map can only lag reality, never lead it. A document staged in an unpublished release does NOT count in either direction — it is not on the master ref, so it is invisible to both the map and the query.
