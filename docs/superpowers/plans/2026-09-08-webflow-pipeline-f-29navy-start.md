...
```bash
curl -s https://<repo>.cdn.prismic.io/api/v2 | python3 -c "import sys,json;print(json.load(sys.stdin)['types'])"
```

The type is listed once it is registered and absent before, so a listed type plus that build error means the document is missing, not the model. A document staged in an unpublished release does NOT count and does not change this — it is not on the master ref, so it is invisible to both the `types` map and the query.
