...
describe("buildCatalogPlan — feeds ride the plan", () => {
  ...
  it("merges entity documents, custom types, diagnostics, and media", () => {
    ...
    // catalog_page always ships first; entity extension types append.
    expect(plan.customTypes.map((c) => c.id)).toEqual(["catalog_page", "person"]);
  ...
  it("without feeds the plan carries only the catalog_page type (no entity types)", () => {
    const plan = buildCatalogPlan(pages, ir);
    expect(plan.documents.map((d) => d.type)).toEqual(["catalog_page"]);
    // The catalog_page type always ships; with no feeds no entity types append.
    expect(plan.customTypes.map((c) => c.id)).toEqual(["catalog_page"]);
  });
});
