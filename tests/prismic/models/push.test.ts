...
//
// It used to live here, as an AST allow-list over push.ts alone: "names only the
// pure diff helper and its types, by ANY import mechanism", plus an ambient
// free-identifier list, a token census and a verb check. All of it is now in
// `tests/prismic/models/index.test.ts`, run over EVERY file in
// `src/prismic/models/` at once, and this copy is deleted rather than kept in
// sync.
//
// That is the point, not tidiness. The version here survived twelve attack
// shapes and still left one hop: `./diff.js` was on its allow-list, so IO added
// inside `diff.ts` and reached through `withRemoteScreenshots` looked clean from
// push.ts, and `diff.ts` had no guard of its own. Two guards have a seam; one
// guard over the whole directory does not. The specific property this file used
// to assert — that push.ts needs no IO capability of ITS OWN, because `send` is
// injected — is carried there by PURE_CORE, which is closed: the comparison core
// may name only the comparison core.
//
// The behavioural half of the property stays here, where it belongs: "never
// sends anything for a remote-only model" above is about what pushModels DOES,
// and no source-level allow-list can assert that.
